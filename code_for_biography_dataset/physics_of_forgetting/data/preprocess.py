...
ATTRIBUTE_OUTPUT_ROOT_DIR = "./data/processed_final/attribute"
BIOGRAPHY_OUTPUT_ROOT_DIR = "./data/processed_final/biography"
TEMPLATE_OUTPUT_ROOT_DIR = "./template/processed_final/"
QA_OUTPUT_ROOT_DIR = "./data/processed_final/qa/"


class DataProcessor:
    @staticmethod
    def process_name_gender(
        first_name_count: int,
        middle_name_count: int,
        last_name_count: int,
        fullname_count: int,
        ...
        while len(fullname_index_set) < fullname_count:
            ...
            fullname_index_set.add(
                (first_name_index, middle_name_index, last_name_index)
            )
        fullname_gender_dict = {}
        for i, (first_name_index, middle_name_index, last_name_index) in enumerate(
            fullname_index_set
        ):
        ...
        if previous_person_name_gender_file_path is not None:
            ...
            previous_full_name_gender_dict = json.load(
                open(previous_person_name_gender_file_path, "r")
            )
            ...
            full_name_gender_dict_reverse = {
                str(value): key for key, value in fullname_gender_dict.items()
            }
            for key, value in tqdm(
                previous_full_name_gender_dict.items(), desc="Phase 2"
            ):
                name_info = {
            ...
            for full_name_gender_dict_key in tqdm(
                full_name_gender_dict_reverse.values(), desc="Phase 3"
            ):
                name_gender_info = fullname_gender_dict[full_name_gender_dict_key]
                name_gender_info["gender"] = (
                    "male" if random.random() < 0.5 else "female"
                )
                reordered_full_name_gender_dict[
                    len(reordered_full_name_gender_dict)
                ] = name_gender_info
        ...
        if not os.path.exists(
            os.path.join(ATTRIBUTE_OUTPUT_ROOT_DIR, "selected_person_name_component")
        ):
            os.mkdir(
                os.path.join(
                    ATTRIBUTE_OUTPUT_ROOT_DIR, "selected_person_name_component"
                )
            )
        json.dump(
            first_name_dict,
            open(
                os.path.join(
                    ATTRIBUTE_OUTPUT_ROOT_DIR,
                    "selected_person_name_component/first_name.json",
                ),
                "w",
            ),
            indent=4,
        ...
        json.dump(
            ...
            open(
                os.path.join(
                    ATTRIBUTE_OUTPUT_ROOT_DIR,
                    "selected_person_name_component/middle_name.json",
                ),
                "w",
            ),
            indent=4,
        ...
        json.dump(
            ...
            open(
                os.path.join(
                    ATTRIBUTE_OUTPUT_ROOT_DIR,
                    "selected_person_name_component/last_name.json",
                ),
                "w",
            ),
            indent=4,
        ...
        json.dump(
            fullname_gender_dict,
            open(
                os.path.join(ATTRIBUTE_OUTPUT_ROOT_DIR, "person_name_gender.json"), "w"
            ),
            indent=4,
        )
    ...
    def process_birthday(begin_year: int, end_year: int):
        ...
        json.dump(
            birthday_dict,
            open(os.path.join(ATTRIBUTE_OUTPUT_ROOT_DIR, "birthday.json"), "w"),
            indent=4,
        )

    @staticmethod
    def process_city(count: int):
        city_dataset = pd.read_csv(
            "./data/raw/city/List_of_United_States_cities_by_population_2.csv"
        )
        city_dict = {}
        ...
        json.dump(
            city_dict,
            open(os.path.join(ATTRIBUTE_OUTPUT_ROOT_DIR, "city.json"), "w"),
            indent=4,
        )
    ...
    def process_university(count: int):
        university_dataset_list = [
            pd.read_csv(
                "./data/raw/university/List_of_research_universities_in_the_United_States_1.csv"
            ),
            pd.read_csv(
                "./data/raw/university/List_of_research_universities_in_the_United_States_2.csv"
            ),
            pd.read_csv(
                "./data/raw/university/List_of_research_universities_in_the_United_States_3.csv"
            ),
        ]
        university_dict = {}
        ...
        json.dump(
            university_dict,
            open(os.path.join(ATTRIBUTE_OUTPUT_ROOT_DIR, "university.json"), "w"),
            indent=4,
        )
    ...
    def process_major(count: int):
        ...
        json.dump(
            major_dict,
            open(os.path.join(ATTRIBUTE_OUTPUT_ROOT_DIR, "major.json"), "w"),
            indent=4,
        )

    @staticmethod
    def process_company(count: int):
        company_dataset = pd.read_csv(
        ...
        json.dump(
            company_dict,
            open(os.path.join(ATTRIBUTE_OUTPUT_ROOT_DIR, "company.json"), "w"),
            indent=4,
        )
...
class TemplateProcessor:
    ...
    def process(template_count: int):
        ...
        for params in [
            (
                "./template/raw/1_birthday.txt",
                os.path.join(TEMPLATE_OUTPUT_ROOT_DIR, "1_birthday.json"),
                "<<BIRTHDAY>>",
            ),
            (
                "./template/raw/2_birth_city.txt",
                os.path.join(TEMPLATE_OUTPUT_ROOT_DIR, "2_birth_city.json"),
                "<<BIRTH_CITY>>",
            ),
            (
                "./template/raw/3_college.txt",
                os.path.join(TEMPLATE_OUTPUT_ROOT_DIR, "3_college.json"),
                "<<COLLEGE>>",
            ),
            (
                "./template/raw/4_major.txt",
                os.path.join(TEMPLATE_OUTPUT_ROOT_DIR, "4_major.json"),
                "<<MAJOR>>",
            ),
            (
                "./template/raw/5_company_name.txt",
                os.path.join(TEMPLATE_OUTPUT_ROOT_DIR, "5_company_name.json"),
                "<<COMPANY_NAME>>",
            ),
            (
                "./template/raw/6_company_city.txt",
                os.path.join(TEMPLATE_OUTPUT_ROOT_DIR, "6_company_city.json"),
                "<<COMPANY_CITY>>",
            ),
        ]:
            params = params + (template_count,)
            TemplateProcessor.process_helper(*params)
        university_template_dict = json.load(
            open(os.path.join(TEMPLATE_OUTPUT_ROOT_DIR, "3_college.json"), "r")
        )
        for k in university_template_dict:
            assert "<<COLLEGE>>" in university_template_dict[k]
            university_template_dict[k] = university_template_dict[k].replace(
                "<<COLLEGE>>", "<<UNIVERSITY>>"
            )
        os.remove(os.path.join(TEMPLATE_OUTPUT_ROOT_DIR, "3_college.json"))
        json.dump(
            university_template_dict,
...
class DatasetConstructor:
    def __init__(
        ...
        self.person_name_gender_dict = json.load(
            open(person_name_gender_file_name, "r")
        )
        ...
        self.birth_city_template_dict = json.load(
            open(birth_city_template_file_name, "r")
        )
        self.university_template_dict = json.load(
            open(university_template_file_name, "r")
        )
        self.major_template_dict = json.load(open(major_template_file_name, "r"))
        self.company_name_template_dict = json.load(
            open(company_name_template_file_name, "r")
        )
        self.company_city_template_dict = json.load(
            open(company_city_template_file_name, "r")
        )
        ...
        self.company_name_qa_template = open(
            company_name_qa_template_file_name, "r"
        ).read()
        self.company_city_qa_template = open(
            company_city_qa_template_file_name, "r"
        ).read()
    ...
    def construct_biography_index(self, previous_index_file_path=None):
        ...
        for i in trange(len(self.person_name_gender_dict)):
            ...
            distinct_biography_entries_count = (
                5  # the maximum number of distinct biography entries for each person
            )
            birthday_template_index_list = random.sample(
                range(0, len(self.birthday_template_dict)),
                distinct_biography_entries_count,
            )
            birth_city_template_index_list = random.sample(
                range(0, len(self.birth_city_template_dict)),
                distinct_biography_entries_count,
            )
            university_template_index_list = random.sample(
                range(0, len(self.university_template_dict)),
                distinct_biography_entries_count,
            )
            major_template_index_list = random.sample(
                range(0, len(self.major_template_dict)),
                distinct_biography_entries_count,
            )
            company_name_template_index_list = random.sample(
                range(0, len(self.company_name_template_dict)),
                distinct_biography_entries_count,
            )
            company_city_template_index_list = random.sample(
                range(0, len(self.company_city_template_dict)),
                distinct_biography_entries_count,
            )
            template_order_list = []
    ...
    def get_person_info(self, person_index: str):
        return {
            "person_name": self.person_name_gender_dict[
                self.index_file[person_index]["person_name"]
            ]["fullname"],
            "birthday": self.birthday_dict[self.index_file[person_index]["birthday"]][
                "birthday"
            ],
            "birth_city": self.city_dict[self.index_file[person_index]["birth_city"]][
                "fullname"
            ],
            "university": self.university_dict[
                self.index_file[person_index]["university"]
            ],
            "major": self.major_dict[self.index_file[person_index]["major"]],
            "company_name": self.company_dict[self.index_file[person_index]["company"]][
                "name"
            ],
            "company_city": self.company_dict[self.index_file[person_index]["company"]][
                "city_fullname"
            ],
        }

    def construct_biography_entry(
        self, person_index: str, template_index: str, permute: bool, fullname: bool
    ):
        template_dict = {
            "birthday": self.birthday_template_dict[
                self.index_file[person_index]["template_index_dict"][template_index][
                    "birthday"
                ]
            ],
        ...
        gender = self.person_name_gender_dict[
            self.index_file[person_index]["person_name"]
        ]["gender"]
        if gender == "female":
            for key in template_dict:
                template_dict[key] = (
                    template_dict[key].replace("his", "her").replace("him", "her")
                )
        current_biography = " "
        token_info = {}
        permuted_attribute_list = []
        if permute:
            order = self.index_file[person_index]["template_index_dict"][
                template_index
            ]["order"]
        ...
        for attribute_name in permuted_attribute_list:
            ...
            first_attribute_token = self.tokenizer(" " + person_info[attribute_name])[
                "input_ids"
            ][0]
            current_biography = current_biography.replace(
                pattern, person_info[attribute_name]
            )
            # check whether the tokenization result is correct
            current_biography_token_list = self.tokenizer(current_biography)[
                "input_ids"
            ]
            assert (
                current_biography_token_list[:first_attribute_token_position]
                == prefix_token_list
            )
            assert (
                current_biography_token_list[first_attribute_token_position]
                == first_attribute_token
            )
            # write token info
    ...
    def construct_biography(
        self, permute: bool, fullname: bool, multi: int, output_file_name: str
    ):
        # single is multi1
        result_dict = {}
        for person_index in trange(len(self.index_file), desc=output_file_name):
            for template_index in range(multi):
                biography = self.construct_biography_entry(
                    str(person_index), str(template_index), permute, fullname
                )
    ...
    def construct_qa(self, train_ratio: float = None):
        ...
        for person_index in tqdm(self.index_file):
            ...
            for pattern, template in [
                ...
                qa_dict[info_key] = {"prompt": prompt, "answer": answer}
        ...
        if train_ratio is None:
            json.dump(
                train_dataset,
                open(os.path.join(QA_OUTPUT_ROOT_DIR, "all.json"), "w"),
                indent=4,
            )
        else:
            json.dump(
                train_dataset,
                open(os.path.join(QA_OUTPUT_ROOT_DIR, "train.json"), "w"),
                indent=4,
            )
            json.dump(
                test_dataset,
                open(os.path.join(QA_OUTPUT_ROOT_DIR, "test.json"), "w"),
                indent=4,
            )
...
def main():
    ...
    constructor = DatasetConstructor(
        ...
        person_name_gender_file_name=os.path.join(
            ATTRIBUTE_OUTPUT_ROOT_DIR, "person_name_gender.json"
        ),
        university_file_name=os.path.join(ATTRIBUTE_OUTPUT_ROOT_DIR, "university.json"),
        birthday_template_file_name=os.path.join(
            TEMPLATE_OUTPUT_ROOT_DIR, "1_birthday.json"
        ),
        birth_city_template_file_name=os.path.join(
            TEMPLATE_OUTPUT_ROOT_DIR, "2_birth_city.json"
        ),
        university_template_file_name=os.path.join(
            TEMPLATE_OUTPUT_ROOT_DIR, "3_university.json"
        ),
        major_template_file_name=os.path.join(TEMPLATE_OUTPUT_ROOT_DIR, "4_major.json"),
        company_name_template_file_name=os.path.join(
            TEMPLATE_OUTPUT_ROOT_DIR, "5_company_name.json"
        ),
        company_city_template_file_name=os.path.join(
            TEMPLATE_OUTPUT_ROOT_DIR, "6_company_city.json"
        ),
    ...
    # NOTE: this line is for generating pretraining data
    constructor.construct_biography(True, True, 5, "multi5_permute_fullname.json")
    # NOTE: this line is for generating finetuning data
    constructor.construct_qa()
    pass
