...
# ===========================================================================
# LOAD CONFIGURATIONS
def get_params(default_cfg_path="./config/default_config.yaml",is_run_in_ipynb=False):
    ...
    # =========================================================================================
    # Wandb
    parser.add_argument("--is_wandb", default=False, type=STR2BOOL, help="if using wandb")
    parser.add_argument("--wandb_project", type=str, default=None, help="wandb project name")
    parser.add_argument("--wandb_entity", type=str, default=None, help="wandb entity name")

    # Config
    parser.add_argument("--cfg", default=default_cfg_path, help="Hyper-parameters") # debug: "./config/default_config.yaml"
    ...
    parser.add_argument("--dump_path", type=str, default="experiments", help="Experiment saved root path (Do not need to be specified explicitly)")
    
    # Saving Checkpoints
    parser.add_argument("--save_llm_ckpt", type=STR2BOOL, default=False, help="If saving checkpoints of the LLM backbone")
    parser.add_argument("--save_llm_ckpt_best", type=STR2BOOL, default=False, help="If saving the best checkpoints of the LLM backbone")
    parser.add_argument("--save_llm_ckpt_epoch_interval", type=int, default=-1, help="Saving checkpoints of the LLM backbone for every n epochs")
    parser.add_argument("--save_llm_ckpt_step_interval", type=int, default=-1, help="Saving checkpoints of the LLM backbone for every n steps")
    parser.add_argument("--save_probing_classifiers", type=STR2BOOL, default=False, help="If saving probing classifiers")
    parser.add_argument("--save_features_before_after_IL", type=STR2BOOL, default=False, help="If saving training features before and after IL")
    ...
    parser.add_argument("--load_llm_ckpt", type=STR2BOOL, default=False, help="If loading checkpoints of the LLM backbone")
    ...
    parser.add_argument("--backbone_max_new_token_list", type=list, default=None, help="The max new tokens for generation for each task. If it is None, all task will be set to backbone_max_new_token.")
    parser.add_argument("--backbone_random_init", default=False, type=STR2BOOL, help="if randomly initialized model parameters")
    ...
    parser.add_argument("--metric_for_each_task", type=list, default=None, help="The customized evaluate metric for each task. ('None' represents all metric is accuracy)")
    # =========================================================================================
    # Training Settings
    # =========================================================================================
    
    parser.add_argument("--is_evaluate_only", default=False, type=STR2BOOL, help="If only evaluate the model instead of incremental training?")
    ...
    parser.add_argument("--max_seq_length_list", type=list, default=None, help="Max length for each sentence for each task. If it is None, all tasks will be set to max_seq_length.") 

    parser.add_argument("--is_probing", default=False, type=STR2BOOL, help="If calculate probing performance for each evaluation")
    ...
    parser.add_argument("--gradient_accumulation_steps", type=int, default=1, help="The step for gradient accumulation")
    ...
    parser.add_argument("--training_epochs_first_task", type=int, default=None, help="The number training epochs for the first task. 'None' means that the value is set to 'training_epochs'.")
    parser.add_argument("--training_epochs_list", type=list, default=None, help="The number training epochs for each task. 'None' means that the value is set to 'training_epochs'. When it is not None, this parameter is piror to training_epochs_first_task and training_epochs.")
    ...
    parser.add_argument("--is_evaluate_init", type=STR2BOOL, default=True, help="If evaluating the model at the initial state (task_id==0,epoch_id==0)")
    ...
    parser.add_argument("--il_mode", default="CIL", choices=["CIL","TIL","IIL","CIT","CKE"], 
                        help="the mode of incremental learning ( Class / Task / Instance Incremental Learning/ Continual Instruction Tuning )")
    ...
    parser.add_argument("--is_replay", default=False, type=STR2BOOL, help="if using experience replay")
    parser.add_argument("--Replay_buffer_size", type=int, default=100, help="The number of samples for data replay (used exclusively with Replay_buffer_ratio)")
    parser.add_argument("--Replay_buffer_ratio", type=float, default=0.01, help="The ratio of samples for data replay (used exclusively with Replay_buffer_size)")
    parser.add_argument("--Replay_batch_level", type=STR2BOOL, default=True, 
    ...
    parser.add_argument("--Replay_fix_budge_each_class", type=STR2BOOL, default=False, 
