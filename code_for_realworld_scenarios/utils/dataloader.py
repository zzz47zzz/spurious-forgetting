...
def get_dataloader_for_sentence_level_dataset_default(params, CL_dataset, tokenizer):
    ...
    def preprocess_function_train_generative(examples):
        ...
        for i in range(len(examples[input_column])):
            _input = examples[input_column][i].strip()
            _target = examples[target_column][i].strip()
    ...
    for task_id in range(num_task):
        ...
        if params.max_seq_length_list is not None:
            setattr(params,'max_seq_length',params.max_seq_length_list[task_id])
        ...
        if params.il_mode == 'IIL':
            ...
            if params.classifier == 'None':
                dev_dataset.set_format(type='torch', columns=['input_ids','attention_mask','target'])
            else:
                dev_dataset.set_format(type='torch', columns=['input_ids','attention_mask',
                                                            'label_idx_cil','label_idx_til','target'])
...
def preprocess_function_train_generative_LAMOL(examples,params,tokenizer,num_task,task_id,input_column,target_column,ans_token,eos_token,gen_token):
    ...
    for i in range(len(examples[input_column])):
        ...
        with_ans_prompted_input_list.append(get_prompt_LAMOL(text=_input,
                                                        ...
                                                        ans_token=ans_token,
                                                        eos_token=eos_token))
        if params.LAMOL_use_eos_as_gen_token:
            _gen_token = tokenizer.eos_token
        else:
    ...
    if params.LAMOL_use_eos_as_gen_token:
        _gen_token_list = [tokenizer.eos_token for t_id in range(num_task)]
    else:
        _gen_token_list = ['__%d__'%(t_id) if params.LAMOL_use_task_specific_gen_token else gen_token for t_id in range(num_task)]
    gen_token_input_ids = tokenizer(_gen_token_list)['input_ids']
...
def get_dataloader_for_sentence_level_dataset_LAMOL(params, CL_dataset, tokenizer):
    ...
    if params.LAMOL_use_eos_as_gen_token:
        assert tokenizer.eos_token is not None, "params.LAMOL_use_eos_as_gen_token is True"
        gen_token = tokenizer.eos_token
    elif params.LAMOL_use_task_specific_gen_token:
...
def get_dataloader_for_word_level_dataset_default(params, CL_dataset, tokenizer):
    ...
    def preprocess_function_train_predict_discriminative(examples):
        ...
        for i in range(len(examples[input_column])):
            x_list, y_list = [], []
            one_sent_x = examples[input_column][i].strip()
