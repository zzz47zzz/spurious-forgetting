...
DATASET2TYPE = {
    ...
    'concept_400_task1': 'sentence-level',
    'concept_400_task10': 'sentence-level',
...
DATASET2MAXLEN = {
    ...
    'concept_400_task1': 32,
    'concept_400_task10': 32,
...
def get_dataset(params):
    ...
    if params.classification_type == 'auto':
        if params.dataset not in DATASET2TYPE.keys():
            logger.warning(f'Dataset {params.dataset} is not pre-defined in DATASET2TYPE, set to sentence-level by default!')
    ...
    if params.max_seq_length == -1:
        if params.dataset not in DATASET2MAXLEN.keys():
            logger.warning(f'Dataset {params.dataset} is not pre-defined in DATASET2MAXLEN, set to 32 by default!')
            max_seq_length = 32
        else:
