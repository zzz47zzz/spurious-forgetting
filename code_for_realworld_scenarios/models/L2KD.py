...
def get_L2KD_params(parser):
    '''
        The parameters of model L2KD
    '''

    parser.add_argument("--L2KD_distill_weight", type=float, default=1.0, help="The weight of the distillation term in L2KD")
    parser.add_argument("--L2KD_lambda", type=float, default=0.25, help="The weight of the generation target in L2KD")
    parser.add_argument("--L2KD_gamma", type=float, default=0.20, help="The ratio of psesudo old samples w.r.t the training data of new task.")
    parser.add_argument("--L2KD_topk", type=int, default=20, help="The top-k sampling for generating psesudo old samples.")
    parser.add_argument("--L2KD_temperature", type=float, default=2.0, help="The temperature for knowledge distillation.")
...
class L2KD(BaseLearner):
    '''
        L2KD: L2KD proposes to add knowledge distillation targets based on LAMOL. 
        The implementation is based on [this repository](https:// github.com/voidism/L2KD).
        We only implemented Word-Level (Word-KD) varation in this respository since it has the best performance on text classification tasks.

        - [Lifelong Language Knowledge Distillation](https://aclanthology.org/2020.emnlp-main.233/)
    '''
    ...
    def begin_task(self, task_id):
        ...
        if task_id>0:
            ...
            for _ in range(self.params.training_epochs):
                ...
                # Training with Causal Language Modeling Loss
                    ...
                    qa_loss = teacher_model(**{'input_ids':lm_input['input_ids_with_ans'], 
                                            'attention_mask':lm_input['attention_mask_with_ans'],
                                            'labels':lm_input['labels_with_ans']}).loss

                    generation_loss = teacher_model(**{'input_ids':lm_input['input_ids_with_gen_ans'], 
                                            'attention_mask':lm_input['attention_mask_with_gen_ans'],
                                            'labels':lm_input['labels_with_gen_ans']}).loss

                    total_loss = qa_loss + self.params.L2KD_lambda*generation_loss
                    ...
                    if not(np.isnan(scalar_loss)) or not(np.isinf(scalar_loss)):
                        ...
                        teacher_loss_list.append(scalar_loss)
    ...
    def train_epochs(self, task_id):
        ...
        if task_id>0:
            ...
            pseudo_buf_dataset_list = self.generate_pseudo_buffer_samples(task_id=task_id,
                                                                     num_samples=int(len(train_dataset)*self.params.L2KD_gamma))
    ...
    def observe_batch(self, task_id, epoch_id, lm_input):
        ...
        if task_id == 0:
            ...
            if torch.sum(cur_mask)==0:
                ...
                # For current task's data (knowledge distillation)
                with torch.no_grad():
                    qa_teacher_logits = teacher_model(**{'input_ids':lm_input['input_ids_with_ans'][cur_mask], 
                                        'attention_mask':lm_input['attention_mask_with_ans'][cur_mask],
                                        'labels':lm_input['labels_with_ans'][cur_mask]}).logits
                    qa_teacher_logits = qa_teacher_logits[lm_input['labels_with_ans'][cur_mask]!=-100]

                    generation_teacher_logits = teacher_model(**{'input_ids':lm_input['input_ids_with_gen_ans'][cur_mask], 
                                        'attention_mask':lm_input['attention_mask_with_gen_ans'][cur_mask],
                                        'labels':lm_input['labels_with_gen_ans'][cur_mask]}).logits
                    generation_teacher_logits = generation_teacher_logits[lm_input['labels_with_gen_ans'][cur_mask]!=-100]

                qa_student_logits = model(**{'input_ids':lm_input['input_ids_with_ans'][cur_mask], 
                                        'attention_mask':lm_input['attention_mask_with_ans'][cur_mask],
                                        'labels':lm_input['labels_with_ans'][cur_mask]}).logits
                qa_student_logits = qa_student_logits[lm_input['labels_with_ans'][cur_mask]!=-100]

                generation_student_logits = model(**{'input_ids':lm_input['input_ids_with_gen_ans'][cur_mask], 
                                        'attention_mask':lm_input['attention_mask_with_gen_ans'][cur_mask],
                                        'labels':lm_input['labels_with_gen_ans'][cur_mask]}).logits
                generation_student_logits = generation_student_logits[lm_input['labels_with_gen_ans'][cur_mask]!=-100]

                temp = float(self.params.L2KD_temperature)

                qa_loss_cur = self.kl_loss(F.log_softmax(qa_student_logits/temp,dim=-1),
                                        F.softmax(qa_teacher_logits/temp,dim=-1))*(temp**2)

                generation_loss_cur = self.kl_loss(F.log_softmax(generation_student_logits/temp,dim=-1),
                                        F.softmax(generation_teacher_logits/temp,dim=-1))*(temp**2)

                total_loss_cur = qa_loss_cur + self.params.L2KD_lambda*generation_loss_cur

            buf_mask = (lm_input['label_idx_cil']==-1)
            if torch.sum(buf_mask)==0:
                total_loss_buf = torch.tensor(0.).to(model.device)
            else:
                # For pseudo buffer data (causal language modeling)
                qa_loss_buf = model(**{'input_ids':lm_input['input_ids_with_ans'][buf_mask], 
                                        'attention_mask':lm_input['attention_mask_with_ans'][buf_mask],
                                        'labels':lm_input['labels_with_ans'][buf_mask]}).loss

                generation_loss_buf = model(**{'input_ids':lm_input['input_ids_with_gen_ans'][buf_mask], 
                                        'attention_mask':lm_input['attention_mask_with_gen_ans'][buf_mask],
                                        'labels':lm_input['labels_with_gen_ans'][buf_mask]}).loss
                total_loss_buf = qa_loss_buf + self.params.L2KD_lambda*generation_loss_buf 
            
            total_loss = total_loss_buf + self.params.L2KD_distill_weight*total_loss_cur
