...
def get_SEQ_params(parser):
    ...
    parser.add_argument("--SEQ_fix_old_classifier", default=False, type=STR2BOOL, help="if fix the old classifiers")
    parser.add_argument("--SEQ_fix_encoder", default=False, type=STR2BOOL, help="if fix the encoder")
    parser.add_argument("--SEQ_freeze_component_list", type=list, 
                        default=['layers.1'],
                        # ['mlp.dense_4h_to_h','mlp.dense_h_to_4h','attention.dense','attention.query_key_value','embed_in','embed_out'] 
                        help="The component for freeze")
    parser.add_argument("--SEQ_freeze_bg_task_id", type=int, default=1, help="The task id begin freezing.")
    ...
    parser.add_argument("--SEQ_preallocated_classifier", type=STR2BOOL, default=False, help="If pre-allocating classifiers for optimization?")
    parser.add_argument("--SEQ_use_prototype_for_prediction", type=STR2BOOL, default=False, help="If using prototype as classifier weights")
...
class SEQ(BaseLearner):
    ...
    def __init__(self, params, CL_dataset, accelerator): 
        ...
        assert params.il_mode in ['IIL','CIL','TIL','CIT','CKE'], 'NotImplemented for il mode %s and model %s'%(params.il_mode,'SEQ')
    ...
    def build_metric(self):
        ...
        if self.params.il_mode in ['IIL', 'CKE']:
            # NOTE: Under CKE (Continual Knowledge Editing) scneario, 
            # result_summary_train records the result of the accuracy of the edit data
            # result_summary records the result of the accuracy of the rephrase data
    ...
    def build_classifier(self):
        if hasattr(self.model,'module'):
            feature_dim = self.model.module.config.hidden_size 
        ...
        out_dim_list = self.CL_dataset.continual_config['CUR_NUM_CLASS'] if self.params.classifier != 'None' else None
    ...
    def begin_task(self, task_id):
        ...
        if task_id >= self.params.SEQ_freeze_bg_task_id:
    ...
    def observe_batch(self, task_id, epoch_id, lm_input):
        ...
        with self.accelerator.accumulate(model):
            ...
            if self.params.classifier == 'None' or \
                ...
                # TODO: to add mamba
                if 'mamba' in self.params.backbone:
                    lm_logits = model(input_ids=lm_input['input_ids_with_ans']).logits
                    shift_logits = lm_logits[:, :-1, :].contiguous()
                    labels = lm_input['labels_with_ans'][:, 1:].contiguous()
                    total_loss = self.ce_loss(shift_logits.view(-1, shift_logits.size(-1)), labels.view(-1))
                else:
        ...
        if self.params.save_llm_ckpt and self.params.save_llm_ckpt_step_interval!=-1 \
            ...
            # FIXME: there is some bug when using Zero Stage 3 ...
            self.accelerator.wait_for_everyone()
            unwrapped_model = self.accelerator.unwrap_model(self.model)
            unwrapped_model.save_pretrained(os.path.join(self.params.dump_path,'checkpoint_llm_task_%d_epoch_%d_step_%d'%(task_id,epoch_id,self.global_step)),
            ...
            #     unwrap_model.save_pretrained(os.path.join(self.params.dump_path,'checkpoint_llm_task_%d_epoch_%d_step_%d'%(task_id,epoch_id,self.global_step)))
    ...
    def end_epoch(self, task_id, epoch_id):
        ...
        if (self.params.evaluate_interval>0) and epoch_id%self.params.evaluate_interval==0:
            ...
            if dev_score > self.best_score:
                ...
                if self.params.save_llm_ckpt and self.params.save_llm_ckpt_best:
                    ...
                    self.accelerator.wait_for_everyone()
                    unwrapped_model = self.accelerator.unwrap_model(self.model)
                    unwrapped_model.save_pretrained(os.path.join(self.params.dump_path,'checkpoint_llm_task_%d_epoch_best'%(task_id)),
                                                    is_main_process=self.accelerator.is_main_process,
                                                    save_function=self.accelerator.save,
                                                    state_dict=self.accelerator.get_state_dict(self.model),
                                                    )
                    ...
                    # if self.accelerator.is_main_process:
                    #     unwrap_model = self.accelerator.unwrap_model(self.model)
                    #     unwrap_model.save_pretrained(os.path.join(self.params.dump_path,'checkpoint_llm_task_%d_epoch_best'%(task_id)))
        ...
        if self.params.save_llm_ckpt and self.params.save_llm_ckpt_epoch_interval!=-1 \
            ...
            self.accelerator.wait_for_everyone()
            unwrapped_model = self.accelerator.unwrap_model(self.model)
            unwrapped_model.save_pretrained(os.path.join(self.params.dump_path,'checkpoint_llm_task_%d_epoch_%d'%(task_id,epoch_id)),
                                            is_main_process=self.accelerator.is_main_process,
                                            save_function=self.accelerator.save,
                                            state_dict=self.accelerator.get_state_dict(self.model),
                                            )
            ...
            # if self.accelerator.is_main_process:
            #     unwrap_model = self.accelerator.unwrap_model(self.model)
            #     unwrap_model.save_pretrained(os.path.join(self.params.dump_path,'checkpoint_llm_task_%d_epoch_%d'%(task_id,epoch_id)))
    ...
    def evaluate_current_task(self,
        ...
        '''
            ...
            Params: 
                ...
                - il_mode: 'IIL' or 'CIL' or 'TIL' or 'CIT' or 'CKE'
        ...
        assert il_mode in ['IIL','CIL','TIL','CIT','CKE'], 'NotImplemented for il_mode %s'%(il_mode)
        if phase=='train':
