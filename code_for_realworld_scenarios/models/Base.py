...
class BaseLearner(object):
    ...
    def incremental_training(self):
        ...
        num_epoch_each_task = self.params.training_epochs
        ...
        for task_id in range(num_task):
            if self.accelerator.is_main_process: 
                ...
                logger.info("============================================================================")
            if self.params.training_epochs_list is not None:
                self.params.training_epochs = self.params.training_epochs_list[task_id]
            elif task_id == 0 and self.params.training_epochs_first_task is not None:
                self.params.training_epochs = self.params.training_epochs_first_task
            else:
                self.params.training_epochs = num_epoch_each_task
    ...
    def end_task(self, task_id):
        ...
        if il_mode in ['IIL','CKE'] and self.params.method != 'ICL':
        ...
        # save ckpt
        if self.params.save_llm_ckpt:

            # Save and upload
            # FIXME: there is some bug when using Zero Stage 3 ...
            self.accelerator.wait_for_everyone()
            unwrapped_model = self.accelerator.unwrap_model(self.model)
            unwrapped_model.save_pretrained(os.path.join(self.params.dump_path,'checkpoint_llm_task_%d_epoch_last'%(task_id)),
            ...
            #     unwrap_model.save_pretrained(os.path.join(self.params.dump_path,'checkpoint_llm_task_%d_epoch_last'%(task_id)))
    ...
    def evaluate_model(self, task_id: int) -> dict:
        ...
        if il_mode in ['IIL','CKE'] and self.params.method != 'ICL':
            ...
            # NOTE: Evaluate the locality for continual knowledge editing using dev set (only)
            # if il_mode == 'CKE':
            #     acc_list = self.evaluate_all_seen_task_tc(cur_task_id, 'dev', il_mode)
            #     result_dict['Locality_Acc'] = np.mean(acc_list)
            #     if self.accelerator.is_main_process:
            #         logger.info('Mode = %s, Locality_Acc = %s'%(il_mode, result_dict['Locality_Acc']))
    ...
    def evaluate_all_seen_task_tc(self,
        ...
        if self.params.classification_type == 'sentence-level':
            ...
            for eval_t_id in range(cur_task_id+1):
                ...
                # testing
                if self.accelerator.is_main_process:
                    logger.info(f"Evaluating task {eval_t_id}...")

                if il_mode=='IIL':
                    acc, save_dict = self.evaluate_current_task(eval_t_id, cur_task_id, phase, il_mode, return_dict=False)
                    ...
                    acc = self.evaluate_current_task(eval_t_id, cur_task_id, phase, il_mode, return_dict=True)
                    if not isinstance(acc,float):
                        assert isinstance(acc[0],float) and isinstance(acc[1],dict)
                        acc, save_dict_all = acc

                acc_list.append(acc)

            # save_dict_all = [save_dict_all] # NOTE: transform to list, otherwise gather_object() will not collect correctly
            # gathered_save_dict_all = gather_object(save_dict_all) 
            # NOTE: We gather in the evaluate_current_task function.
            gathered_save_dict_all = save_dict_all

            if self.accelerator.is_main_process:
                if gathered_save_dict_all is not None:
                    with open(os.path.join(self.params.dump_path,
                                        f'{phase}_cur_task_{cur_task_id}_global_step_{self.global_step}_save_result.npy'),'wb') as f:
    ...
    def evaluate_current_task(self,
        ...
        '''
            ...
            Params: 
                ...
                - phase: 'train','dev'or'test'
                - il_mode: 'CIL' or 'TIL'
                - return_dict: If return the prediction dict (for IIL and SafetyAlignment dataset)
